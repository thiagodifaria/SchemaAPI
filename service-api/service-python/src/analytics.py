...
from learn.feedback import feedback_analyzer
from learn.retrain import retrainer
from learn.temporal import temporal_analyzer
from template.create import template_create
...
def run_temporal_analysis():
    ...
    try:
        patterns = temporal_analyzer.detect_recurring_topics(conn)
        if not patterns:
    ...
    try:
        with conn.cursor() as cur:
            ...
            for version_id, content_bytes in versions_to_process:
                ...
                result = template_detect.extract_features(text)
                cur.execute(
...
def run_feedback_analysis():
    conn = get_db_connection()
    try:
        aggregated_errors, processed_ids = feedback_analyzer.analyze_feedback(conn)
...
def run_template_creation():
    conn = get_db_connection()
    try:
        created_templates = template_create.create_templates_from_structures(conn)
...
def run_retraining_job():
    conn = get_db_connection()
    try:
        retrainer.trigger_retraining(conn)
    except Exception as e:
