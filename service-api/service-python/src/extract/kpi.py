...
class KPIExtractor:
    def __init__(self):
        ...
        self.kpi_patterns = [
            re.compile(r'(Receita\s*Líquida|Receita\s*Liquida|Net\s*Revenue)\s*(?:foi\s*)?de\s*(R\$|\$|USD)\s*([\d.,]+)\s*(milhões|milhão|mil|bi|bilhões|billion|million)?', re.IGNORECASE),
            re.compile(r'(Lucro\s*Bruto|Gross\s*Profit)\s*(?:foi\s*)?de\s*(R\$|\$|USD)\s*([\d.,]+)\s*(milhões|milhão|mil|bi|bilhões|billion|million)?', re.IGNORECASE),
            re.compile(r'(EBITDA)\s*(?:foi\s*)?de\s*(R\$|\$|USD)\s*([\d.,]+)\s*(milhões|milhão|mil|bi|bilhões|billion|million)?', re.IGNORECASE),
