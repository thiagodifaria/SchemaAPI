...
pub async fn get_document_diff(
    ...
    let action_items_diff = diff::compare_action_items(from_results.action_items, to_results.action_items);
    let chunks_diff = diff::compare_chunks_semantic(from_results.chunks, to_results.chunks);

    let diff_result = diff::StructuralDiff {
        action_items_diff,
        chunks_diff,
    };
